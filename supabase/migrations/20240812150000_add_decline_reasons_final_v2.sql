-- Final, Triple-Checked Migration to Add Decline Reasons
-- This version corrects the RLS policy by using a subquery to check for user participation.
...
-- RLS Policies for the new table (Corrected with Subquery)
CREATE POLICY "Users can view decline reasons for referrals they are part of"
    ...
    USING (
        referral_id IN (
            SELECT r.id FROM referrals r WHERE r.from_user_id = auth.uid() OR r.to_user_id = auth.uid()
        )
    );
...
CREATE POLICY "Users can insert a decline reason for a referral they are part of"
    ...
    WITH CHECK (
        ...
        referral_id IN (
            SELECT r.id FROM referrals r WHERE r.to_user_id = auth.uid()
        )
    );
