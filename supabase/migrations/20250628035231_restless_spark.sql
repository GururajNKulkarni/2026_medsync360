/*
  # Fix research table constraints and search indexes

  1. Changes
    - Add unique constraints to research tables (if they don't already exist)
    - Recreate full-text search indexes with proper configuration
    
  2. Implementation
    - Check if constraints exist before adding them
    - Drop and recreate search indexes for better performance
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'research_news_title_key' 
    AND conrelid = 'research_news'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'research_drugs_name_key' 
    AND conrelid = 'research_drugs'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'research_cases_title_key' 
    AND conrelid = 'research_cases'::regclass
  ) THEN
