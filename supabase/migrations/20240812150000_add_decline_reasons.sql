...
ADD COLUMN decline_reason_id UUID REFERENCES referral_decline_reasons(id);
...
CREATE POLICY "Users can view decline reasons for referrals they are part of"
    ...
    USING (
        referral_id IN (
            SELECT id FROM referrals WHERE from_user_id = auth.uid() OR to_user_id = auth.uid()
        )
    );
...
CREATE POLICY "Users can insert a decline reason for a referral they are part of"
    ...
    WITH CHECK (
        ...
        referral_id IN (
            SELECT id FROM referrals WHERE to_user_id = auth.uid()
        )
    );
