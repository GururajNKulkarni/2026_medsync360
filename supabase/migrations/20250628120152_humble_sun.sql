/*
  # Add Referral Attachment Improvements
  ...
  3. Changes
    - Add functions to handle attachment previews and downloads
    - Add functions to track and retrieve decline reasons
*/
...
-- Create policy for reading decline reasons
...
-- Create policies for referral status history
CREATE POLICY "Users can read status history for their referrals"
...
-- Create function to record status change with reason
...
-- Create function to get decline reasons for a referral
...
-- Create function to get all available decline reasons
CREATE OR REPLACE FUNCTION get_decline_reasons()
RETURNS TABLE (
  id uuid,
  code text,
  label text,
...
BEGIN
  ...
  SELECT 
    ...
    rdr.reason_code,
    rdr.reason_label,
    rdr.reason_description
  FROM referral_decline_reasons rdr
  WHERE rdr.is_active = true
  ORDER BY rdr.reason_label;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION get_decline_reasons() TO authenticated;

-- Create function to generate a signed URL for attachment preview
CREATE OR REPLACE FUNCTION get_attachment_preview_url(
  attachment_id uuid,
...
BEGIN
  ...
  SELECT ra.file_name, ra.file_url
  ...
  -- For now, just return the file_url
  -- In a production environment, this would generate a signed URL with expiry
  RETURN attachment_record.file_url;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION get_attachment_preview_url(uuid, integer) TO authenticated;