...
-- This will copy missing patient information from original referrals to transferred ones
...
WHERE transferred.transfer_parent_id = original.id
  AND (
    transferred.room_no IS NULL 
    OR transferred.patient_ip_no IS NULL 
    OR transferred.patient_admission_time IS NULL 
    OR transferred.past_history IS NULL 
    OR transferred.general_examination IS NULL
  );

-- Log the update as a comment
-- Backfilled missing patient data in transferred referrals from their original referrals
-- This update completed on: now()