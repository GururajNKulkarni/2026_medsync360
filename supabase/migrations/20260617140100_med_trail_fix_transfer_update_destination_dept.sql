-- get_complete_medication_trail(): for an "Updated During Transfer" step, show the
-- ACTUAL transfer destination department instead of the originating referral's own
-- to_department.
--
-- Bug (patient Don, 2026-06-17): a transfer_update is recorded against the referral
-- being transferred FROM. The old context used that referral's to_department (where
-- the transferring doctor already sat), so a transfer Guru (Anaesthesiology) ->
-- Karuna (Cardiology) read "...during transfer to MD Anaesthesiology". The correct
-- destination is the child referral's to_department.
--
-- Only the transfer_update branch of the medication_context CASE changed.
