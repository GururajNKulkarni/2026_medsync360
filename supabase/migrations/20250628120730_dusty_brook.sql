-- Create table for standardized decline reasons if it doesn't exist
...
-- Insert standard decline reasons if they don't exist
...
-- Create table for referral status history with reasons if it doesn't exist
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'referral_status_history' 
    AND policyname = 'Users can read status history for their referrals'
  ) THEN
...
-- Drop function if it exists to avoid errors
...
-- Drop function if it exists to avoid errors
DROP FUNCTION IF EXISTS get_decline_reasons();
